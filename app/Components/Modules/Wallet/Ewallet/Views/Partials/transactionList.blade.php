<div class="transactionList transferWizard overTrans">
    <div class="transferWhite">
        <h2 class="heading"><i class="icon-wallet"></i>{{ _mt('Wallet-Ewallet','Ewallet.transactions') }}</h2>
        <form class="filterForm">
            <div class="filters row">
                <div class="eachFilter operation col-md-4">
                    <label>{{ _mt($moduleId,'Ewallet.operation') }}</label>
                    <select name="filters[operation]">
                        <option value="">{{ _mt($moduleId,'Ewallet.all') }}</option>
            ...
            <div class="filters row">
                <div class="eachFilter transactionType col-md-4">
                    <label>{{ _mt($moduleId,'Ewallet.type') }}</label>
                ...
                <div class="eachFilter groupBy col-md-4">
                    <label>{{ _mt($moduleId,'Ewallet.group_by') }}</label>
                ...
                <div class="eachFilter sortBy col-md-4">
                    <label>{{ _mt($moduleId,'Ewallet.sort_by') }}</label>
            ...
            <div class="filters row">
                <div class="eachFilter orderBy col-md-4">
                    <label>{{ _mt($moduleId,'Ewallet.order_by') }}</label>
                ...
                <div class="filterButtonWrapper col-md-4">
                    <button type="button" class="btn ladda-button dark transactionFilter" data-style="contract">
                        <i class="fa fa-filter"></i>{{ _mt($moduleId,'Ewallet.filter') }}
                    </button>
                </div>
            </div>
        </form>
        <div class="holder">
            @forelse($groupOfTransaction as $key => $group)
                ...
                <span class="groupTag">
                    ...
                    <span class="total">
                    {{ _mt($moduleId,'Ewallet.total_transaction') }}
                    <i>{{ $group->count() }}</i>
                </span>
                ...
                <div class="transactionsHolder toggleBody mfkToggleOuterWrap">
                    @forelse($group as $key => $transaction)
                        ...
                        <div class="eachTransaction {{ $transaction->totalCharges }} mfkToggleWrap">
                            ...
                            <span class="txn">
                                <p>{{ _mt($moduleId,'Ewallet.txn') }}</p>#{{ $transaction->id }}
                            ...
                            <div class="transactionBody toggleBody">
                                ...
                                <div class="col-md-8 totals mfkToggleOuterWrap">
                                    <div class="eachSection mfkToggleWrap">
                                        <h3 class="mfkToggle">{{ _mt($moduleId,'Ewallet.actual_amount') }}</h3>
                                        <div class="toggleBody">{{ currency($transaction->actual_amount) }}</div>
                                    </div>
                                    <div class="eachSection">
                                        <h3 class="mfkToggle">{{ _mt($moduleId,'Ewallet.transaction_charges') }}</h3>
                                        <div class="toggleBody">
                                            ...
                                            @empty
                                                <div class="noCharges">{{ _mt($moduleId,'Ewallet.no_charges') }}</div>
                                            @endforelse
                                        </div>
                                    </div>
                    ...
                    @empty
                        <div class="noTransactions">{{ _mt($moduleId,'Ewallet.no_transaction_in_this') }} {{ $groupName }}</div>
                    @endforelse
                </div>
            @empty
                <div class="noTransactions">{{ _mt($moduleId,'Ewallet.no_transaction') }}</div>
            @endforelse
        </div>
    </div>
</div>
