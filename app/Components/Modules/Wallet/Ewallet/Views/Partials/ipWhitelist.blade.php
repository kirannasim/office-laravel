...
<div class="whiteListIp">
    <h2>{{ _mt($moduleId,'Ewallet.IP_Restriction') }}</h2>
    ...
    <div class="tabHolder">
        <div class="tab">
            <div class="buttonBase {{ $ipStatus ? 'on' : 'off' }}">
                <span class="on">{{ _mt($moduleId,'Ewallet.on') }}</span>
                <span class="off">{{ _mt($moduleId,'Ewallet.off') }}</span>
...
<script type="text/javascript">
    ...
    $('.updateIp').click(function () {
        ...
        if (!validateIp() && $('.whiteListIp .buttonBase').hasClass('on')) {
            addValidationError({invalidIp: "{{ _mt($moduleId,'Ewallet.IP_is_invalid') }}"});
            Ladda.stopAll();
            return false;
        }

        var post = $.post('{{ route(strtolower(getScope()).".ewallet.validate.ip.whitelist") }}', {
