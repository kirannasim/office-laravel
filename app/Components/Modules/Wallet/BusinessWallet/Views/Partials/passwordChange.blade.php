<form class="passChange col-md-12">
    ...
    <div class="inputs">
        <h2 class="heading">{{ _mt($moduleId,'BusinessWallet.change_transaction_password') }}</h2>
        <div class="firstPart">
            <div class="changePasswordFields">
                <div class="row changePass">
                    <div class="form-group">
                        <label class="control-label col-md-4">{{ _mt($moduleId,'BusinessWallet.new_password') }}
                ...
                <div class="row changePass">
                    <div class="form-group">
                        <label class="control-label col-md-4">{{ _mt($moduleId,'BusinessWallet.re_enter_password') }}
                            <span class="required" aria-required="true"> * </span>
                        </label>
                        <div class="col-md-6">
                            <div class="input-group">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <button type="button" class="btn btn-outline dark ladda-button changePassword"
                            ...
                            {{ _mt($moduleId,'BusinessWallet.update') }}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="successNotice">
                ...
                <h3>{{ _mt($moduleId,'BusinessWallet.tran_password_updated') }}</h3>
            </div>
        </div>
    </div>
</form>
...
<script type="text/javascript">
    ...
    $('button.changePassword').click(function () {
        ...
        var post = $.post('{{ scopeRoute("businesswallet.validate.change_password") }}', formData, function (response) {
            $('.changePasswordFields').slideUp();
            refreshAjaxData($('.transactionPassLoader'), '{!! route(strtolower(getScope()).".businesswallet.tran.password.view") !!}');
            Ladda.stopAll();
    ...
    function validatePasswordForm() {
        ...
        if ((password.length < 8) || (confirmPassword.length < 8)) {
            addValidationError({password: "{{ _mt($moduleId,'BusinessWallet.re_enter_password') }}"});
            return false;
        }
        if (password != confirmPassword) {
            addValidationError({missmatch: "{{ _mt($moduleId,'BusinessWallet.Confirm_Password_Miss_Match') }}"});
            return false;
        }
