<div class="wallet transactionPassWrapper">
    <div class="innerWrap">
        <div class="eachField">
            <label>{{_mt($moduleId,'BusinessWallet.please_enter_your_tran_password') }}</label>
        </div>
        <div class="eachField">
            <input type="password" placeholder="{{_mt($moduleId,'BusinessWallet.enter_tran_password') }}"
...
<script type="text/javascript">
    ...
    $('body').off('click', '.submitTransactionPass').on('click', '.submitTransactionPass', function () {
        ...
        $.post('{{ scopeRoute("businesswallet.submitTransactionPass") }}', options, function (response) {
            Ladda.stopAll();
            if (window.hasOwnProperty('transactionPassSuccessCallback')) {
                window.transactionPassSuccessCallback(response, 'businesswallet');
            } else {
            ...
            switch (response.status) {
                case 403:
                    $('.businesswallet.transactionErrors').empty().append('<div class="error">Session timeout !</div>').slideDown();
                    break;
                case 401:
                    var errors = response.responseJSON;
                    $('.businesswallet.transactionErrors').empty().html('<div class="error">' + errors.error + '</div>').slideDown();
                    break;
                case 422:
                    var errors = response.responseJSON;
