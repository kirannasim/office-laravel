<div class="transactionList transferWizard overTrans">
    <div class="transferWhite">
        <h2 class="heading"><i class="icon-wallet"></i>{{ _mt($moduleId,'BusinessWallet.transactions') }}</h2>
        <form class="filterForm">
            <div class="filters row">
                <div class="eachFilter operation col-md-4">
                    <label>{{ _mt($moduleId,'BusinessWallet.operation') }}</label>
                    <select name="filters[operation]">
                        <option value="">{{ _mt($moduleId,'BusinessWallet.all') }}</option>
            ...
            <div class="filters row">
                <div class="eachFilter transactionType col-md-4">
                    <label>{{ _mt($moduleId,'BusinessWallet.type') }}</label>
                ...
                <div class="eachFilter groupBy col-md-4">
                    <label>{{ _mt($moduleId,'BusinessWallet.group_by') }}</label>
                ...
                <div class="eachFilter sortBy col-md-4">
                    <label>{{ _mt($moduleId,'BusinessWallet.sort_by') }}</label>
            ...
            <div class="filters row">
                <div class="eachFilter orderBy col-md-4">
                    <label>{{ _mt($moduleId,'BusinessWallet.order_by') }}</label>
                ...
                <div class="filterButtonWrapper col-md-4">
                    <button type="button" class="btn ladda-button dark transactionFilter" data-style="contract">
                        <i class="fa fa-filter"></i>{{ _mt($moduleId,'BusinessWallet.filter') }}
                    </button>
                </div>
            </div>
        </form>
        <div class="holder">
            @forelse($groupOfTransaction as $key => $group)
                ...
                <span class="groupTag">
                    ...
                    <span class="total">
                    {{ _mt($moduleId,'BusinessWallet.total_transaction') }}
                    <i>{{ $group->count() }}</i>
                </span>
                ...
                <div class="transactionsHolder toggleBody mfkToggleOuterWrap">
                    @forelse($group as $key => $transaction)
                        @php
                            $additionalAmount = $transaction->isCredit ? (float)($transaction->totalCharges + $transaction->cartTotalAmount) : (float)($transaction->cartTotalAmount - $transaction->totalCharges);
                            $netAmount = $transaction->belongsToExtraIncome ? abs($additionalAmount) : $transaction->actual_amount + $additionalAmount;
                        @endphp
                        <div class="eachTransaction mfkToggleWrap">
                            <div class="transactionHeader mfkToggle">
                            <span class="txn">
                                <p>{{ _mt($moduleId,'BusinessWallet.txn') }}</p>#{{ $transaction->id }}
                                ...
                                @if($transaction->belongsToExtraIncome)
                            ...
                            <div class="transactionBody toggleBody">
                                ...
                                <div class="col-md-8 totals mfkToggleOuterWrap">
                                    <div class="eachSection mfkToggleWrap">
                                        <h3 class="mfkToggle">{{ _mt($moduleId,'BusinessWallet.actual_amount') }}</h3>
                                        <div class="toggleBody">{{ currency($transaction->actual_amount) }}</div>
                                    </div>
                                    <div class="eachSection">
                                        <h3 class="mfkToggle">{{ _mt($moduleId,'BusinessWallet.transaction_charges') }}</h3>
                                        <div class="toggleBody">
                                            @forelse($transaction->charges as $charge)
                                                <div class="charge">
                                                    <span class="name">{{ $charge->module()->model->name }}</span>
                                                    <span class="amount @if($charge->module()->operationType() == 'add') credit @endif">
                                                    {{ $charge->module()->operationType() == 'add' ? '+' : '-' }}{{ currency($charge->amount) }}
                                                </span>
                                                </div>
                                            @empty
                                                <div class="noCharges">{{ _mt($moduleId,'BusinessWallet.no_charges') }}</div>
                                            @endforelse
                                        </div>
                                    </div>
                                            ...
                                            <div class="toggleBody">
                                                @forelse($transaction->order->totals as $total)
                                                    <div class="charge">
                                                        ...
                                                        <span class="amount @if($total->module()->operationType() == 'add') credit @endif">
                                    ...
                                    <div class="eachSection total">
                                        <h3>{{ _mt($moduleId,'BusinessWallet.total') }}</h3>
                                        <div class="total">{{ currency($netAmount) }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    @empty
                        <div class="noTransactions">{{ _mt($moduleId,'BusinessWallet.no_transaction_in_this') }} {{ $groupName }}</div>
                    @endforelse
                </div>
            @empty
                <div class="noTransactions">{{ _mt($moduleId,'BusinessWallet.no_transaction') }}</div>
            @endforelse
        </div>
    </div>
</div>
