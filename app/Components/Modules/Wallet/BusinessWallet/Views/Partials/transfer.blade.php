<div class="transferWrap wallet eSend col-md-12">
    <form class="transferWizard">
        ...
        <div class="successNotice">
            ...
            <h3>{{ _mt($moduleId,'BusinessWallet.successfully_sent') }}</h3>
        </div>
        <div class="inputs">
            <h2 class="heading"><i class="icon-paper-plane"></i>{{ _mt($moduleId,'BusinessWallet.send') }}</h2>
                    ...
                    <div class="dataHolder">
                        ...
                        <div class="actions">
                            <button class="addPayee btn blue" type="button">
                                <i class="fa fa-user-plus"></i>{{ _mt($moduleId,'BusinessWallet.add_payee') }}
                            </button>
                            <button class="transferInit ladda-button btn green" data-style="contract" type="button">
                                {{ _mt($moduleId,'BusinessWallet.pay') }}<i class="fa fa-angle-double-right"></i>
                            </button>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $('body').off('click', '.transferInit').on('click', '.transferInit', function () {
        ...
        var post = $.post('{{ route("businesswallet.validate.transfer") }}', formData, function (response) {
            //postPayment(response);
            Ladda.stopAll();
    ...
    function validatePaymentForm() {
        ...
        if (!$('.eachUser').length) {
            addPaymentError({payeeMissing: "{{_mt($moduleId,'BusinessWallet.add_atleaset_one_payee') }}"});
            return false;
        }
        resetPaymentErrors();
