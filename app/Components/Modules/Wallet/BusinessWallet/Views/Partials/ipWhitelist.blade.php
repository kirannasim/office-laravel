@php
    $ip = $walletSettings->get('ip');
    $ipStatus = $walletSettings->get('ip_status');
@endphp

<div class="whiteListIp">
    <h2>{{ _mt($moduleId,'BusinessWallet.IP_Restriction') }}</h2>
    ...
    <div class="tabHolder">
        <div class="tab">
            <div class="buttonBase {{ $ipStatus ? 'on' : 'off' }}">
                <span class="on">{{ _mt($moduleId,'BusinessWallet.on') }}</span>
                <span class="off">{{ _mt($moduleId,'BusinessWallet.off') }}</span>
...
<script type="text/javascript">
    ...
    $('.updateIp').click(function () {
        ...
        if (!validateIp() && $('.whiteListIp .buttonBase').hasClass('on')) {
            addValidationError({invalidIp: "{{ _mt($moduleId,'BusinessWallet.IP_is_invalid') }}"});
            Ladda.stopAll();
            return false;
        }

        var post = $.post('{{ scopeRoute("businesswallet.validate.ip.whitelist") }}', {
