<div class="transferWrap wallet eSend col-md-12">
    <form class="transferWizard">
        ...
        <div class="successNotice">
            ...
            <h3>{{ _mt($moduleId,'IPayWallet.successfully_sent') }}</h3>
        </div>
        <div class="inputs">
            <h2 class="heading"><i class="icon-paper-plane"></i>{{ _mt($moduleId,'IPayWallet.send') }}</h2>
            ...
            <div class="row">
                <div class="dataPrepare col-md-6">
                    <div class="dataHolder">
                        <div class="fieldCover">
                            ...
                            <input type="text" placeholder="{{ _mt($moduleId,'IPayWallet.select_user') }}"
                                   class="userFiller field username">
                        </div>
                        <div class="fieldCover">
                            <span class="icon">{{ currencySymbol() }}</span>
                            <input type="text" name="amount"
                                   placeholder="{{ _mt($moduleId,'IPayWallet.enter_amount') }}"
                        ...
                        @endif
                        <div class="fieldCover">
                                <textarea name="remarks" placeholder="{{ _mt($moduleId,'IPayWallet.remarks') }}"
                                          class="field remarks"></textarea>
                        </div>
                        <div class="actions">
                            <button class="addPayee btn blue" type="button">
                                <i class="fa fa-user-plus"></i>{{ _mt($moduleId,'IPayWallet.add_payee') }}
                            </button>
                            <button class="transferInit ladda-button btn green" data-style="contract" type="button">
                                {{ _mt($moduleId,'IPayWallet.pay') }}<i class="fa fa-angle-double-right"></i>
                            </button>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 amountInfo">
                    <div class="content">
                        <label>{{ _mt($moduleId,'IPayWallet.total_amount') }}</label>
...
<script type="text/javascript">
    ...
    $('body').off('click', '.transferInit').on('click', '.transferInit', function () {
        ...
        var post = $.post('{{ scopeRoute("ipaywallet.validate.transfer") }}', formData, function (response) {
            //postPayment(response);
            Ladda.stopAll();
            ...
            window.paymentSuccessCallback = function (response, wallet) {
                if (wallet != 'ipaywallet') return;
                postPayment(response);
            };
            refreshAjaxData($('.transactionPassLoader'), '{!! scopeRoute("ipaywallet.initPayment") !!}', {'verifyOnly': false});
    ...
    function validatePaymentForm() {
        ...
        if (found) {
            addPaymentError({amountMissing: "{{_mt($moduleId,'IPayWallet.amount_missing_fields') }}"});
            return false;
        }
        if (!$('.eachUser').length) {
            addPaymentError({payeeMissing: "{{_mt($moduleId,'IPayWallet.add_atleaset_one_payee') }}"});
            return false;
        }
        resetPaymentErrors();
