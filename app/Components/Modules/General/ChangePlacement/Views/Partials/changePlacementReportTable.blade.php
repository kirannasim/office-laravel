@if($changedPlacementData->count())
    ...
    <table class="table table-striped table-bordered table-hover dataTable dtr-inline reporttable" role="grid"
        ...
        <tr>
            ...
            <th> {{ _mt($moduleId,'ChangePlacement.sponsor') }}</th>
        ...
        @foreach($changedPlacementData as $placementData)
            <tr>
                ...
                <td> {{ $placementData->user->sponsor()['username']}} </td>
{{--                <td> @if($placementData->user->userSponsor!=null) {{ $placementData->user->userSponsor->username }} @else {{ '' }} @endif </td>--}}
