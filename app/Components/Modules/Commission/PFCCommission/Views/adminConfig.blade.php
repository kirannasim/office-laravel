...
@endforeach
<div class="row moduleConfig PFCComission">
    <div class="col-sm-12">
        {!! Form::open(['route' => ['admin.module.configure',$moduleId],'class' => 'form-horizontal adminConfig','id' => 'adminConfig']) !!}
        <div class="panel panel-primary">
            <div class="panel-heading">
                <span aria-hidden="true"
                      class="icon-tag"></span> PFC Comission
            </div>
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'PFCComission.To_Wallet') }}</label>
                    </div>
                ...
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'PFCComission.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'PFCComission.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                'module_data[distribution_type]': "{{ _mt($moduleId,'PFCComission.Please_choose_distribution_type') }}",
                'module_data[wallet]': "{{ _mt($moduleId,'PFCComission.Please_select_wallet') }}",
            },
        ...
        $('.PFCComission .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'PFCComission.Configuration_saved_sucessfully') }}");
            };
