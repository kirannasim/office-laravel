...
@endforeach
<div class="row moduleConfig DiamondBonusPool">
    <div class="col-sm-12">
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'DiamondBonusPool.To_Wallet') }}</label>
                    </div>
                ...
                <div class="row form-group">
                    <div class="col-md-3">
                        <label>
                            {{ _mt($moduleId, 'DiamondBonusPool.Pool_Share') }} <span class="type"></span>
                        </label>
                    </div>
                ...
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'DiamondBonusPool.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'DiamondBonusPool.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                'module_data[wallet]': "{{ _mt($moduleId,'DiamondBonusPool.Please_select_wallet') }}",
                'module_data[pool_share]': "{{ _mt($moduleId,'DiamondBonusPool.Please_enter_pool_share') }}",
            },
        ...
        $('.DiamondBonusPool .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'DiamondBonusPool.Configuration_saved_successfully') }}");
            };
