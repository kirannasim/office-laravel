...
@endforeach
<div class="row moduleConfig GenerationMatchingBonus">
    <div class="col-sm-12">
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'GenerationMatchingBonus.To_Wallet') }}</label>
                    </div>
                    ...
                    <div class="col-md-4 col-xs-4">
                        <div class="col-md-5">
                            <input type="button" id="addLevel" class="btn btn-sm green"
                                   value="{{ _mt($moduleId,'GenerationMatchingBonus.Add_Level') }}"/>
                        </div>
                        <div class="col-md-5">
                            <input type="button" id="removeLevel" class="btn btn-sm red"
                                   value="{{ _mt($moduleId,'GenerationMatchingBonus.Remove_Level') }}"/>
                        </div>
                    </div>
                </div>
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'GenerationMatchingBonus.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'GenerationMatchingBonus.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                'module_data[distribution_type]': "{{ _mt($moduleId,'GenerationMatchingBonus.Please_choose_distribution_type') }}",
                'module_data[wallet]': "{{ _mt($moduleId,'GenerationMatchingBonus.Please_select_wallet') }}",
            },
        ...
        $('.GenerationMatchingBonus .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'GenerationMatchingBonus.Configuration_saved_sucessfully') }}");
            };
