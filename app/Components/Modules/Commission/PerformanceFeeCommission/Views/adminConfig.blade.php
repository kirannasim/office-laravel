...
@endforeach
<div class="row moduleConfig PerformanceFeeCommission">
    <div class="col-sm-12">
        {!! Form::open(['route' => ['admin.module.configure',$moduleId],'class' => 'form-horizontal adminConfig','id' => 'adminConfig']) !!}
        <div class="panel panel-primary">
            <div class="panel-heading">
                <span aria-hidden="true"
                      class="icon-tag"></span> Performance Fee Commission [PFC]
            </div>
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'PerformanceFeeCommission.To_Wallet') }}</label>
                    </div>
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-3 form-group">
                        <input type="number" class="form-control input-small"
                               name="module_data[percentage]" value="{{ $config->get('percentage') }}">
                    </div>
                </div>
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> 5 Start Pool Share</label>
                    </div>
                    <div class="col-md-3 form-group">
                        <input type="number" class="form-control input-small"
                               name="module_data[star_pool_share]" value="{{ $config->get('star_pool_share') }}">
                    </div>
                </div>
                <div class="row form-group" id="level-value">
                    ...
                    <div class="col-md-6 col-xs-6">
                        <div class="col-md-3">
                            <input type="button" id="addLevel" class="btn btn-sm green"
                                   value="{{ _mt($moduleId,'PerformanceFeeCommission.Add_Level') }}"/>
                        </div>
                        <div class="col-md-4">
                            <input type="button" id="removeLevel" class="btn btn-sm red"
                                   value="{{ _mt($moduleId,'PerformanceFeeCommission.Remove_Level') }}"/>
                        </div>
                    </div>
                </div>
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'PerformanceFeeCommission.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'PerformanceFeeCommission.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    "use strict";
    var iCnt = {{ count($config['commission']) }}
        //to add more levels
        $('#addLevel').click(function () {
            iCnt = iCnt + 1;
            $('#level-value').append('<div class="row col-md-12" id="field_level_' + iCnt + '" style="width: 100%;overflow: hidden;"> <div class="col-md-3"> <label>Level ' + iCnt + ' : </label> </div> <div class="col-md-3" style="padding: 0px 7px;"> <label> <input type="text" class="form-control input-small" name="module_data[commission][level_' + iCnt + ']" id="level_' + iCnt + '" value="0" /></label> </div> </div>');
        });
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                'module_data[distribution_type]': "{{ _mt($moduleId,'PerformanceFeeCommission.Please_choose_distribution_type') }}",
                'module_data[wallet]': "{{ _mt($moduleId,'PerformanceFeeCommission.Please_select_wallet') }}",
            },
        ...
        $('.PerformanceFeeCommission .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'PerformanceFeeCommission.Configuration_saved_sucessfully') }}");
            };
