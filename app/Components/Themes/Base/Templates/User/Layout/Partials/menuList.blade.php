...
<li class="dd-item" data-id="{{ $menu['id'] }}" data-parent_id="{{ $menu['parent_id'] }}">
    ...
    <form class="menuData">
        ...
        <div class="eachfield row currentLanguage">
            ...
            <div class="col-md-9">
                <input type="text" data-lang="{{ currentLocal() }}" class="name"
                       value="{{ $menu['label'][currentLocal()] }}" placeholder="Menu name"
                       name="menus[leftmenu][{{ $menu['id'] }}][label][{{ currentLocal() }}]">
                <button type="button" class="btn green showAllLanguages">All languages</button>
            </div>
        </div>
