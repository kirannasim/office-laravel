...
@section('content')
    ...
    <div class="memberSearch col-md-offset-3">
        <div class="searchActions">
            ...
            <div class="advFilters row">
                {{--
                <div class="eachFilter operation col-md-3 col-sm-3">
                    <label>{{ _t('member.package') }}</label>
                    <select name="filters[package]" class="package_filter">
                        <option value="">{{ _t('member.select_package') }}</option>
                ...
                <div class="eachFilter operation col-md-3 col-sm-3">
                    <label>{{ _t('member.rank') }}</label>
                    <select name="filters[rank]" class="advnc_rank_filter">
                        <option value="">{{ _t('member.select_ranks') }}</option>
                ...
                --}}
                <div class="eachFilter operation col-md-3 col-sm-3">
                    <label>{{ _t('member.member_id') }}</label>
                    <input type="text" name="filters[memberId]" class="form-control member_id_filter"
                           placeholder="{{ _t('member.Enter_Member_ID') }}">
                </div>
            </div>
        </div>
    ...
    <div class="memberManagementWrapper row">
        <div class="memberNav col-md-3 col-sm-3">
            @component('Admin.Management.Member.Components.memberCard')
                {{ $user->id }}
            @endcomponent
        </div>
    ...
    <script>
        ...
        $(function () {
            ...
            $('button.advSearch').click(function () {
                var options = {
                    ...
                    rank: $('.advnc_rank_filter').val(),
                    memberId: $('.member_id_filter').val(),

                };
                $.post('{{ route('management.members.filter') }}', options, function (response) {
                    console.log(response);
                    Ladda.stopAll();
                    $('.advSearchResult').slideDown().find('.resultHolder').empty().siblings('.resultSummary')
                        .find('h3 span').text(response.length);
                    if (!response.length) {
                        $('.advSearchResult .resultHolder')
                            .html("<div class='noAdvSearchResults'>{{ _t('member.no_results') }}</div>");
                    }
                    array_chunk(response, 4).forEach(function (set) {
                        ...
                        $('.advSearchResult .resultHolder').append('</div>' + output);
                    });
            ...
            $('body').on('click', '.setHolder .resultSet', function () {
                ...
                getUserCard(user).done(function () {
                    loadUserUnit(user, true).done(function () {
        ...
        function loadUserUnit(user, append) {
            let targetSlice = $('.slice[data-user="' + user + '"]');
        ...
        function loadUnit(elem, postParams, appendOrPrepend, unit, route) {
            ...
            route = route ? route : '{{ route("management.members.unit") }}';
        ...
        function setupUserSearch() {
            let input = $('.memberSearch .inputHolder input[type="text"]');
            let options = {
        ...
        function getUserCard(userId) {
            let targetCard = $('.navCard[data-user="' + userId + '"]');
